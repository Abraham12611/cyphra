...
export default async function handler(
  ...
  try {
    ...
    const formattedBadges = reputationStore.badges.map((badge) => ({
      ...
      timestamp: new Date(Number(badge.timestamp) * 1000).toISOString(),
    }));
    ...
    return res.status(200).json({
      ...
      reputation: {
        address,
        reputation_score: reputationStore.reputation_score,
        badges: formattedBadges,
        contribution_count: reputationStore.contribution_count,
        successful_payments: reputationStore.successful_payments,
        has_store: true,
